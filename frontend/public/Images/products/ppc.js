...
document.addEventListener('DOMContentLoaded', function() {
 ...
 const marquee1 = document.getElementById('marquee1');
    ...
    // Set the duration of the first marquee (in milliseconds)
    const marqueeDuration = 20000; // Adjust this to the actual duration

    // Smooth transition after the first marquee completes
    setTimeout(() => {
        // Fade out the first marquee and title
        marquee1.classList.add('hidden');
        title1.classList.add('hidden');

        // Wait for the fade-out transition (1 second) before showing the next elements
        setTimeout(() => {
            // Hide title1 and marquee1
            title1.style.display = 'none';
            marquee1.style.display = 'none';

            // Show title2 and marquee2
            title2.style.display = 'block';
            marquee2.classList.remove('hidden');
            marquee2.classList.add('visible');
