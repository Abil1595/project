                ...
                <Fragment>
                    <div className="row d-flex justify-content-around">
                        <div className="col-12 col-lg-8 mt-5 order-details">
                            ...
                            {/* Display order times based on status */}
                            <h4 className="my-4">Order Timing:</h4>
                            <p>
                                <b>Order At:</b> {new Date(createdAt).toLocaleString()}
                            </p>

                            {orderStatus === "Delivered" && (
                                <p>
                                    <b>Delivered At:</b> {deliveredAt ? new Date().toLocaleString() : "Pending"}
                                </p>  
                            )}
                                ...
                                {orderItems && orderItems.map(item => (
                                    <div className="row my-5" key={item.product}>
                                        ...
                                        <div className="col-5 col-lg-5">
                                            <Link to={`/product/${item.product}`}>{item.name}</Link> {/* Corrected this line */}
                                        </div>

                                        <div className="col-4 col-lg-2 mt-4 mt-lg-0">
                                            <p>${item.offerprice?item.offerprice:item.price}</p>
                                        </div>
